// Janus — Cloudflare Worker：HMAC 签名反向代理 + 内嵌工具页
//
// 部署：
// 1. Workers → Create → 粘贴本文件 → Deploy
// 2. Settings → Variables → Secret：SECRET_KEY（仅用于校验代理请求）
// 3. Compatibility date ≥ 2024-09-02（支持非标出站端口）
// 4. 绑定自定义域名
//
// 访问逻辑：
//   带 d + t → 代理转发（验签后去掉 d/t）
//   否则 GET → 工具主页（链接转换 / 订阅转换，纯浏览器端运算）
...
<main>
  ...
  <section id="panel-sub" class="panel card">
    <p class="lead" style="margin:0 0 12px;font-size:14px">粘贴 <code>vless://</code> 链接（每行一条），或 base64 订阅内容。仅支持 VLESS。转换后将使用当前 Worker 域名作为代理地址。</p>
    <label>订阅链接
      <textarea id="vlessIn" spellcheck="false" placeholder="vless://uuid@example.com:9443?..."></textarea>
    </label>
    <div class="actions">
      <button type="button" class="action primary" id="btnVless">转换</button>
      <button type="button" class="action" id="btnClear">清空</button>
    </div>
    <div id="vlessWarnings"></div>
    <p class="status" id="vlessStatus"></p>
    <div class="result" id="vlessResult">
      <div class="row"><h3>转换结果</h3><button type="button" class="action" data-copy="vlessOut">复制</button></div>
      <div class="output" id="vlessOut"></div>
    </div>
  </section>
...
function splitVless(text){
  const kept=[],skipped=[];
  for(const line of decodeSub(text).split(/\\r?\\n/).map(x=>x.trim()).filter(Boolean)){
    if(/^vless:\\/\\//i.test(line))kept.push(line);
...
async function convertOne(link,secret,proxy){
  ...
  return{ok:true,text:"vless://"+id+"@"+proxy+":443?"+params.toString()+(frag?"#"+frag:"")};
}

$("btnVless").addEventListener("click",async()=>{
  const raw=$("vlessIn").value.trim(),secret=secretEl.value,proxy=location.hostname;
  const st=$("vlessStatus"),res=$("vlessResult"),warn=$("vlessWarnings");
  if(!raw){setStatus(st,"请输入订阅链接。","error");return}
  if(!secret){setStatus(st,"请填写密钥。","error");return}
  const{kept,skipped}=splitVless(raw);
  if(skipped.length){
    const c=new Map();skipped.forEach(p=>c.set(p,(c.get(p)||0)+1));
    warn.innerHTML=[...c].map(([p,n])=>'<div class="warning">已忽略 '+n+' 条 '+esc(p)+' 链接；本工具当前仅支持转换 VLESS 链接。</div>').join("");
  }else warn.innerHTML="";
  if(!kept.length){setStatus(st,"没有可处理的 vless:// 链接。","error");res.classList.remove("visible");return}
  $("btnVless").disabled=true;setStatus(st,"转换中…");res.classList.remove("visible");
  try{
    const out=[],errs=[];
    ...
    if(out.length){$("vlessOut").textContent=out.join("\\n");res.classList.add("visible")}
  ...
  finally{$("btnVless").disabled=false}
});
$("btnClear").addEventListener("click",()=>{$("vlessIn").value="";$("vlessWarnings").innerHTML="";setStatus($("vlessStatus"),"");$("vlessResult").classList.remove("visible")});
